describe('i18n helper', () => {
  ...
  test('ignores runtime overrides that only repeat the translation key', () => {
    jest.isolateModules(() => {
      const i18n = require('../../../src/i18n');

      i18n.registerDictionary('zh-CN', {
        'menubar.menu.logout': 'menubar.menu.logout',
      });
  ...
  test('initializes from window.APP_SETTINGS when available', () => {
    jest.isolateModules(() => {
      ...
      expect(global.window.APP_SETTINGS.i18n['menubar.menu.logout']).toBe('安全退出');
